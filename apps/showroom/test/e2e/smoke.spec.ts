...
test("catalog and detail routes render", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "Audit Plan Descartes" })).toBeVisible();

  await page.goto("/skills/audit-plan-descartes");
  await expect(page.getByRole("heading", { level: 1, name: "Audit Plan Descartes" })).toBeVisible();
