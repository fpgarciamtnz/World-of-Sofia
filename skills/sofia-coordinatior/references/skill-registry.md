...
- `pattern-code-wittgenstein` owns pre-implementation repository and Maxi-context precedent discovery.
...
- `ui-attention-kahneman` owns automatic rendered UI attention and language-fit validation for UI-changing work.
...
|---|---|---|---|---|
| `sofia-coordinatior` | global or project-local | The user invokes `$sofia-coordinatior`, asks to coordinate Sofia skills, route a workflow, create/update/review a skill, explore AI limits, challenge over-safe defaults, enforce audit gates, or show `Sofia trace` / `skill contributions`. | Load this skill first, identify the phase and posture, then select specialists from this registry. If trace visibility is requested after a Sofia-led plan, append one response-level `Skill Impact Trace`. | Routing owner. Handoff to specialist skills instead of absorbing their work. Owns opt-in post-plan contribution summaries without claiming runtime instrumentation. |
...
| Skill | Location | Trigger | How to call | Relationship and handoff |
|---|---|---|---|---|
| `sofia-coordinatior` | project-local | World-of-Sofia skill routing, skill creation/update plans, boldness posture handoffs, multi-skill workflow coordination, or requested `Sofia trace` / `skill contributions`. | Load `skills/sofia-coordinatior/SKILL.md`, then select the smallest specialist sequence and posture. If trace visibility is requested after a Sofia-led plan, append one response-level `Skill Impact Trace`. | Coordinator owner. Produces posture handoffs, role handoffs, routed plans or reviews, and opt-in post-plan contribution summaries without claiming runtime instrumentation. |
...
| `pattern-code-wittgenstein` | project-local | World-of-Sofia precedent discovery, pattern scout work, reusable context checks before creating new code, or evaluation of that project skill. | Load `skills/pattern-code-wittgenstein/SKILL.md` before implementation pattern choices; when new code or a new solution is being created, include its Maxi-context reuse checkpoint if the Maxi handoff is available. | Pre-implementation precedent owner. Combines current-repo evidence with Maxi evidence without treating external snapshots as binding local rules. If code already changed, hand off to Hegel instead. |
...
| `ui-attention-kahneman` | project-local | Any UI-changing work reaches rendered validation, final review, or completion; also use for screenshots, local pages, or mockups that need attention, hierarchy, CTA copy, or emotional language fit. | Load `skills/ui-attention-kahneman/SKILL.md` after rendered evidence exists. | Automatic UI validation owner. Routes missed UI outcomes back through Sofia. |
...
- UI implementation: plan -> implementation -> rendered evidence -> `ui-attention-kahneman` -> `earned-presence-kondo` if competing elements may need demotion/removal -> fixes if needed.
